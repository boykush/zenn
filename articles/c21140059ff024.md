...
2023年5月から、Wikiリンク記法向けの静的サイトジェネレーターCLIを作っています。Zennでも過去に何度か紹介をしてきました。
...
最初は、Markdownファイル同士を `[[Wikiリンク]]` でつなぎ、ドキュメント群を静的サイトとして出力するためのCLIとして作り始めました。その後、MarkdownナレッジベースをAIエージェントから扱えるようにするため、MCPサーバーやプラグイン、Skillsを追加し、読む・探す・書き足すための連携を少しずつ拡充してきました。

Markdownを人間が読むための静的サイトにするだけでなく、AIエージェントが扱える知識基盤としても使えるようにする。その方向性は見えてきた一方で、このツールを何として位置づけるのかは、しばらく言語化できずにいました。
