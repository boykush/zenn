...
topics: ["cli", "mise", "toml"]
published: true
---

:::message alert
2026/07追記: その後の mise の動向（bootstrap 機能の登場）を反映した続編記事を書きました
...
これまでグローバルな開発ツールに対しては、　`mise use -g node` のように動的にインストールをすることが多かったのですが、[mise Registry](https://mise.jdx.dev/registry.html)に登録されているCLIツールは全てbrewからmise設定ファイル管理に移行しました。
...
:::message
Renovateによるバージョン更新を有効にするには、`bat = "0.26.1"` ではなく `"aqua:sharkdp/bat" = "0.26.1"` のようにバックエンド（aqua, asdf, npm等）込みでツールを指定する必要があります。
:::
