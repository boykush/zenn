...
例として[Renovateはmise設定ファイルのバージョン更新に対応している](https://docs.renovatebot.com/modules/manager/mise/)ので、静的バージョン管理を行うことができます。プロジェクト単位のmise設定ファイルでは実践済みでしたが今回初めてグローバルに導入しました。
