...
こんにちは。タイトルの通りOSSとして開発しているWiki向け静的サイトジェネレーター（以下、SSG）のご紹介です。

:::message
Zennのスクラップ機能とは関連はありません！ややこしくすみません
:::
...
後ほど紹介しますが、Scrapsのメインの軸は**軽量**であることです。
...
:::message
メタデータ記述を批判したいわけではありません！色々な多機能SSGで便利に使っています（zenn-cliにもある機能ですが為念）
:::
...
Done in 8.81 secs
```
自身のナレッジサイトではScrap300件弱に対して8.81sec程度です。なるべく自身が一番のヘビーユーザーになり性能劣化が起きないよう注視していきます。
